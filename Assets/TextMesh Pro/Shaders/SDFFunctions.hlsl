// Ray tracing passes generated by Shader Graph include this file twice, with an
// early '#include_with_pragmas' that lands before the material cbuffer and
// sampler declarations, so referencing TMP material properties (_Bevel*,
// _Reflectivity, ...), _Time or screen-space derivatives from here breaks those
// passes. The include guard swallows the duplicate include, and the
// SHADER_STAGE_RAY_TRACING branch keeps every function self-contained:
// ray-traced text simply loses bevel/specular/UV animation. Raster passes
// compile the original code, unchanged.
#ifndef TMP_SDF_FUNCTIONS_INCLUDED
#define TMP_SDF_FUNCTIONS_INCLUDED
...
{
	nornmal = float3(0.0, 0.0, isFront ? 1.0 : -1.0);
}
...
{
	outUV = inUV * transform.xy + transform.zw;
}
...
#endif // TMP_SDF_FUNCTIONS_INCLUDED

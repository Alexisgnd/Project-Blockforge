...
// =========================================================
// AJOUT DES 5 PROPULSEURS A MODELE 3D (Lynx -> Cheetah)
// A L'INVENTAIRE
// Menu : Blockforge > Setup Thruster Blocks
// - Resynchronise les assets BlockDefinition sur la liste
//   canonique (les 5 Block_Thruster_N* recoivent leur FBX
...
[InitializeOnLoad]
public static class ThrusterBlockSetup
{
    private const string ScenePath = "Assets/_Project/Scenes/Garage.unity";
    private const string AutoRunMarker = "Library/BlockforgeThrusterBlocks.done";

    static ThrusterBlockSetup()
    {
    ...
    [MenuItem("Blockforge/Setup Thruster Blocks")]
    public static void SetupMenu()
    {
        Run();
    }

    // Point d'entree batch (Unity -batchmode -executeMethod ThrusterBlockSetup.BatchRunAll) :
    // materiaux + blocs en une passe, puis marqueurs poses pour ne pas rejouer a l'ouverture.
    public static void BatchRunAll()
    {
        ThrusterMaterialSetup.Setup();
        if (Run())
            File.WriteAllText(AutoRunMarker, "done");
        File.WriteAllText("Library/BlockforgeThrusterMaterials.done", "done");
    }
    ...
    private static bool Run()
    {
        ...
        if (controller != null)
        {
            ...
            Debug.Log($"[ThrusterBlockSetup] {blocks.Length} blocs relies a l'inventaire de la scene " +
        ...
        if (controller == null)
        {
            Debug.LogError("[ThrusterBlockSetup] GarageInventoryController introuvable dans la scene " +
        ...
        Debug.Log($"[ThrusterBlockSetup] {blocks.Length} blocs relies a l'inventaire ; scene Garage sauvegardee.");
        return true;
    }
}

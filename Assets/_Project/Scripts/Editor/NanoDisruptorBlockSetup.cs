...
// =========================================================
// AJOUT DES 3 DISTRIBUTEURS NANO A MODELE 3D (Blinder -> Constructor)
// A L'INVENTAIRE
// Menu : Blockforge > Setup Nano Disruptor Blocks
// - Resynchronise les assets BlockDefinition sur la liste
//   canonique (les 3 Block_Nano_N* recoivent leur FBX
...
[InitializeOnLoad]
public static class NanoDisruptorBlockSetup
{
    private const string ScenePath = "Assets/_Project/Scenes/Garage.unity";
    private const string AutoRunMarker = "Library/BlockforgeNanoDisruptorBlocks.done";
    private const string MaterialsMarker = "Library/BlockforgeNanoDisruptorMaterials.done";

    static NanoDisruptorBlockSetup()
    {
    ...
    [MenuItem("Blockforge/Setup Nano Disruptor Blocks")]
    ...
    // Point d'entree batch (Unity -batchmode -executeMethod NanoDisruptorBlockSetup.BatchRunAll) :
    // materiaux + blocs en une passe, puis marqueurs poses pour ne pas rejouer a l'ouverture.
    public static void BatchRunAll()
    {
        NanoDisruptorMaterialSetup.Setup();
    ...
    private static bool Run()
    {
        ...
        if (controller != null)
        {
            ...
            Debug.Log($"[NanoDisruptorBlockSetup] {blocks.Length} blocs relies a l'inventaire de la scene " +
        ...
        if (controller == null)
        {
            Debug.LogError("[NanoDisruptorBlockSetup] GarageInventoryController introuvable dans la scene " +
        ...
        Debug.Log($"[NanoDisruptorBlockSetup] {blocks.Length} blocs relies a l'inventaire ; scene Garage sauvegardee.");
        return true;
    }
}
